...
export default class ExponentialE {
    input: number[];
    output: number[];
    ...
    train() {
        ...
        this.b = sumXY / sumXX;
        this.a = Comp.e ** (meanLnY - this.b * meanX);

        this.predictedOutput = Comp.unique(this.input).map(x => Comp.roundTo(this.a * (Comp.e ** (this.b * x))));
    ...
    predict(x: number) {
        ...
        return Comp.roundTo(this.a * (Comp.e ** (this.b * x)));
    }
};